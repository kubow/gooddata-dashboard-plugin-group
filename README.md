# GoodData.UI Dashboard Plugin - vis-grouped-date

This plugin modifies existing line chart continuing the line with predicted data. `$WIDGET_TITLE_SUFFIX` (defined in `Plugin.jsx`).

## How to make it work

1. Provide backend, workspace and dashboard in `.env`
2. Rename `.template.env.secrets` to `.env.secrets` and provide your preferred way of authentication
3. Install the dependencies `yarn install`
4. Add `Day` date type granularity for trendBy on a line chart
5. Add `_grouped_date` suffix to insights' titles on the dashboard
6. run `yarn start` to develop
7. See instructions below how to build, deploy, and use this plugin
...
2.  Develop your plugin code in `src/dp_vis_grouped_date`

    The `src/dp_vis_grouped_date/Plugin.jsx` is the main plugin file where you have to register all
    your custom content. However, you can create as many new files as you want under the `src/dp_vis_grouped_date`
...
5.  Add plugin to one or more workspaces: `npm run add-plugin -- <url>`
    ...
    Run the `npm run add-plugin -- "https://your.hosting/pluginDirOfYourChoice/dp_vis_grouped_date.js"` to
...
6.  Use plugin on a dashboard: `npm run link-plugin -- <plugin-object-id>`
    ...
    If your plugin supports parameterization (see [src/dp_vis_grouped_date](./src/dp_vis_grouped_date/Plugin.tsx)) and
...
This project is setup so that all your custom code must be self-contained in the [src/dp_vis_grouped_date](./src/dp_vis_grouped_date) directory.

The [src/dp_vis_grouped_date_engine](./src/dp_vis_grouped_date_engine) and [src/dp_vis_grouped_date_entry](./src/dp_vis_grouped_date_entry) directories contain essential plugin boilerplate.
...
You can modify minEngineVersion and maxEngineVersion properties in `src/dp_vis_grouped_date_entry/index`.
